...
internal sealed class ShortcutField : TextBox
{
    ...
    {ReadOnly=true;ShortcutsEnabled=false;BackColor=Color.White;ForeColor=Color.Black;Cursor=Cursors.Hand;}
...
internal sealed class HotkeyDialog : Form
{
    ...
    internal HotkeyDialog(HotkeySettings settings,Func<HotkeySettings,string> apply)
    {
        ...
        StartPosition=FormStartPosition.CenterParent;BackColor=Color.FromArgb(25,28,34);ForeColor=Color.White;Font=new Font("Segoe UI",9);
        var help=new Label{Text="Click a field, then press the desired key or combination.\nFor F7+Up: hold F7, press Up, then release both keys.",Bounds=new Rectangle(16,12,438,40)};Controls.Add(help);
        foreach(HotkeyAction action in Enum.GetValues(typeof(HotkeyAction)))
        {
            int index=(int)action;
            Controls.Add(new Label{Text=HotkeySettings.Label(action),Bounds=new Rectangle(16,60+index*33,200,26),TextAlign=ContentAlignment.MiddleLeft});
        ...
        error.SetBounds(16,359,438,48);error.ForeColor=Color.FromArgb(255,130,130);Controls.Add(error);
